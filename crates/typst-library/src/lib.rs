...
impl Display for Feature {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            ...
            // Fork-local; `Display` is exhaustive, so this arm is required.
            // Deliberately *not* mirrored in `FromStr` below.
            Feature::CndSemantics => "cnd-semantics",
        })
    }
}
...
impl FromStr for Feature {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            // `Feature::CndSemantics` is intentionally absent: this is what
            // parses `--feature <name>`, and the variant changes layout, so
            // it must not be reachable from the PDF/HTML/SVG path. Only
            // `typst-cnd`'s own world may enable it. Do not "complete" this
            // match — see the variant's docs.
            _ => Err(()),
        }
    }
}
